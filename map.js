...
(function () {
  ...
  function focusClosestHub(map, entry, hereLngLat, onSelect) {
    ...
    setTimeout(function () {
      ...
      // fitBounds pads the *coordinate*, but a marker is drawn above its
      // coordinate: the "You are here" pin stands 30px tall anchored at
      // its point, with the label another ~20px above that. Padding the
      // point by the same amount all round therefore clips the label
      // off the top edge — which is exactly the piece this button
      // exists to show. Hence the asymmetric top.
      var pad = 48;
      var top = 64;
      ...
      // no room in it.
      if (pad * 2 >= w - 40 || top + bottom >= h - 40) {
        map.fitBounds(bounds, { padding: 10, maxZoom: 17, duration: 900 });
        return;
      }

      map.fitBounds(bounds, {
        padding: { top: top, bottom: bottom, left: pad, right: pad },
